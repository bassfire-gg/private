...
public class UserControllerTest {
    ...
    void getAllUsersThroughUserDto() throws Exception {
        ...
        mockMvc.perform(get("/api/users"))
                ...
                .andExpect(jsonPath("$._embedded.userResponseList[0].name").value("Jon"))
                .andExpect(jsonPath("$._embedded.userResponseList[1].name").value("Ivan"))
    ...
    void createUserThroughUserDto() throws Exception {
        ...
        mockMvc.perform(post("/api/users")
                        ...
                        .content(userJson("Kate", "kate@mail.ru", 18)))
    ...
    void updateUserThroughUserDto() throws Exception {
        ...
        mockMvc.perform(put("/api/users/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(userJson("Updated", "updated@mail.ru", 35)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(1))
    ...
    private static String userJson(String name, String email, int age) {
        return """
                {"name":"%s","email":"%s","age":%d}
                """.formatted(name, email, age).trim();
    }
}
